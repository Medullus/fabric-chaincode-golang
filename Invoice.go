...
func (t *SimpleChaincode) addInvoices(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	logger.Debugf("adding invoice: %d", len(args))
	...
	for _, v := range items {
		...
		//if unmatched!= nil {
		//	cn,_:=getCN(stub)
		//	newAttr := []string{cn, unmatched.RefID, "org"}
		//	colorNameIndexKey, err := stub.CreateCompositeKey(indexName, newAttr)
		//	logger.Debug("UNMATCH is not nil!")
		//	logger.Debugf("ADD to unmatched:%v with pk %s\n\n", unmatched, colorNameIndexKey)
		//	//logger.Errorf("Unmatched: %s\n%s",indexName, attr)
		//	//value := []byte{0x00}
		//	vBytes, err := json.Marshal(unmatched)
		//
		//	err = stub.PutState(colorNameIndexKey, vBytes)
		//	if err != nil {
